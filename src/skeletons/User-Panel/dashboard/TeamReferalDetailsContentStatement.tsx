          ...
          <table className="w-full border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-[#D9D9D9] rounded-t-lg">
                {["#", "Date", "TID", "Invest Amount", "Income"].map((_, idx) => (
                  <th key={idx} className="text-center py-4 text-black font-medium px-4">
 ...
 <table className="w-full border-collapse min-w-[800px]">
  <thead>
    <tr className="bg-[#D9D9D9] rounded-t-lg">
      ...
      <th className="text-center py-4 text-black font-medium px-4">Invest Amount</th>
      <th className="text-center py-4 text-black font-medium px-4">Income</th>
    </tr>
  </thead>
  <tbody>
    {statements.length > 0 ? (
      statements.map((statement, index) => (
        ...
        >
          ...
          <td className="py-4 text-center px-4 text-[#00FF90]">$ {formatToTwoDecimals(statement.income)}</td>
        </tr>
      ))
    ) : (
      <tr>
        ...
        <div className="mt-4">
          <div className="w-full rounded-xl bg-white text-black p-3 px-4 sm:px-[2rem] flex flex-col sm:flex-row justify-between items-center font-[600] text-sm sm:text-base gap-2 sm:gap-0">
            ...
            <span className="whitespace-nowrap">
              $ {statements.reduce((acc, curr) => acc + (Number(curr.income) || 0), 0)}
            </span>
          </div>
        </div>
      </div>
