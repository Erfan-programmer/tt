...
export default function SendMessageForm({handleReload}:{handleReload: (stauts:boolean) => void}) {
    ...
    <div className="send-message-container px-[1rem] py-[1rem] bg-[#f4f7fd] dark:bg-[var(--sidebar-bg)] bg-shadow-custom border-standard rounded-xl mt-5 pb-[2rem]">
      ...
      <form className="p-2 sm:px-[1rem]" onSubmit={handleSubmit}>
        ...
        <div className="my-4">
          ...
          <input
            type="text"
            className="rounded-[2rem] p-2 border-standard w-full placeholder:text-[#d0d0d0] text-[var(--main-background)] dark:text-white"
        ...
        <div className="my-4">
          ...
          <CKEditor
            ...
            onChange={(_, editor) => handleChange("message", editor.getData())}
          />
          {errors.message && (
          ...
          {formData.attachments.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-2">
              {formData.attachments.map((file, idx) => (
                <span
                  key={idx}
                  className="bg-gray-200 dark:bg-gray-700 rounded px-2 py-1 text-xs flex items-center gap-1"
                >
                  {file.name}
                  <button
                    type="button"
