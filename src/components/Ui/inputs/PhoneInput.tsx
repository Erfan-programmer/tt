...
export default function PhoneInput({
  ...
  // 1. NEW: Create a ref to store the latest onPrefixChange function
  const onPrefixChangeRef = useRef(onPrefixChange);

  // 2. NEW: Update the ref whenever onPrefixChange changes
  useEffect(() => {
  ...
  // 3. LOGIC CHANGE: Removed 'onPrefixChange' from dependencies
  useEffect(() => {
      ...
      try {
        ...
        if (res.success) {
          ...
          if (!selectedCountry && countryData.length > 0) {
            ...
            onPrefixChangeRef.current?.(countryToSelect);
          }
        }
      } catch (err) {
    ...
    fetchCountries();
  }, [debouncedSearch, selectedCountry, defaultDialCode]); // The warning is now gone

  // 5. CHANGE: Use the ref for the select handler as well
  const handleCountrySelect = (country: Country) => {
    setSelectedCountry(country);
    setIsOpen(false);
    onPrefixChangeRef.current?.(country);
  };
