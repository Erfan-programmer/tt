...
function ColorlibStepIcon(props: ColorlibStepIconProps) {
  ...
  const valueColor = icon === 1 ? "#888" : active ? "#65FFD9" : "#888";
      ...
      <Box sx={{ overflowX: "auto", width: "100%" }} className="sidebar-item">
        ...
        >
          {steps.map((label, index) => (
            <Step key={label}>
              <StepLabel
                StepIconComponent={(props) => <ColorlibStepIcon {...props} value={values[index]} />}
                sx={{ "& .MuiStepLabel-label": { color: "#888 !important" } }}
              >
                {label}
              </StepLabel>
            </Step>
