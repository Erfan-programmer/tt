...
export default function ExportFormatBox({
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const res = await apiRequest<any>(
        `${process.env.NEXT_PUBLIC_API_URL}/v1/admin/exportCancellationHistory?wallet_type=${selectedCurrency.network}`,
        "GET",
        undefined,
        {Authorization : `Bearer ${token}`}
      );
  ...
  const handleSendAll = async () => {
    ...
    try {
      const token = loadEncryptedData()?.token;
      const res = await apiRequest<any>(
        `${process.env.NEXT_PUBLIC_API_URL}/v1/admin/cancellations/sendAll`,
        "POST",
        {
          reason: "send all",
        },
