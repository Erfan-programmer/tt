...
export default function CashAwardHistoryRecipientList({
  type,
}: {
  type: "cash" | "physical";
}) {
  ...
  const fetchRecipients = useCallback(async () => {
    ...
    try {
      const endpoint =
        type === "cash"
          ? "/v1/admin/unclaimedPrizeHistoryCash"
          : "/v1/admin/unclaimedPrizeHistoryPhysical";
  ...
  return (
    <>
      <AdminDynamicTable<UnClaimerDataType>
        columns={columns}
        data={recipients}
        title="History of award recipients"
        loading={loading}
      />
